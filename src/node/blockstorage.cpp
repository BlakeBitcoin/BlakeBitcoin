...
bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check native proof here. AuxPoW validation requires an authoritative
    // height and is performed by the CBlockIndex overload below.
    if (!block.IsAuxpow() && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams)) {
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex, const Consensus::Params& consensusParams)
{
    ...
    if (!CheckAuxPowProofOfWork(block, consensusParams, pindex->nHeight)) {
        return error("ReadBlockFromDisk(CBlock&, CBlockIndex*): Errors in block header at %s", block_pos.ToString());
    }
    return true;
}
...
bool BlockManager::ReadBlockHeader(CBlockHeader& block, const CBlockIndex& index) const
{
    ...
    if (filein.IsNull()) {
        return error("ReadBlockHeader: OpenBlockFile failed for %s", block_pos.ToString());
    }
    ...
    if (block.GetHash() != index.GetBlockHash()) {
        return error("ReadBlockHeader: GetHash() doesn't match index for %s at %s",
