...
static CKey MakeKey(const std::string& secret_hex, bool compressed)
{
    CKey key;
...
{
    CKey key1  = MakeKey(SECRET1_HEX, false);
    BOOST_CHECK(key1.IsValid() && !key1.IsCompressed());
    CKey key2  = MakeKey(SECRET2_HEX, false);
    BOOST_CHECK(key2.IsValid() && !key2.IsCompressed());
    CKey key1C = MakeKey(SECRET1_HEX, true);
    BOOST_CHECK(key1C.IsValid() && key1C.IsCompressed());
    CKey key2C = MakeKey(SECRET2_HEX, true);
    BOOST_CHECK(key2C.IsValid() && key2C.IsCompressed());
    CKey bad_key = DecodeSecret(EncodeDestination(CTxDestination(PKHash(key1.GetPubKey()))));
    ...
    BOOST_CHECK(DecodeDestination(EncodeDestination(CTxDestination(PKHash(pubkey1))))  == CTxDestination(PKHash(pubkey1)));
    BOOST_CHECK(DecodeDestination(EncodeDestination(CTxDestination(PKHash(pubkey2))))  == CTxDestination(PKHash(pubkey2)));
    BOOST_CHECK(DecodeDestination(EncodeDestination(CTxDestination(PKHash(pubkey1C)))) == CTxDestination(PKHash(pubkey1C)));
    BOOST_CHECK(DecodeDestination(EncodeDestination(CTxDestination(PKHash(pubkey2C)))) == CTxDestination(PKHash(pubkey2C)));
...
BOOST_AUTO_TEST_CASE(key_signature_tests)
{
    ...
    CKey key = MakeKey(SECRET1_HEX, false);
...
BOOST_AUTO_TEST_CASE(key_key_negation)
{
    ...
    CKey key = MakeKey(SECRET1_HEX, true);
