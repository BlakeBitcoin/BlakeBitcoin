...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    BOOST_CHECK(
        it_giant_tx->second.m_state.GetRejectReason() == "tx-size" ||
        it_giant_tx->second.m_state.GetRejectReason() == "bad-txns-inputs-missingorspent");
...
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    // missing inputs, so the package validation isn't expected to happen.
    {
        ...
        BOOST_CHECK(
            it_parent->second.m_state.GetResult() == TxValidationResult::TX_WITNESS_MUTATED ||
            it_parent->second.m_state.GetResult() == TxValidationResult::TX_NOT_STANDARD);
        BOOST_CHECK(
            it_parent->second.m_state.GetRejectReason() == "bad-witness-nonstandard" ||
            it_parent->second.m_state.GetRejectReason() ==
                "non-mandatory-script-verify-flag (Witness provided for non-witness script)");
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    // Package CPFP: Even though the parent pays 0 absolute fees, the child pays 1 BBTC which is
    ...
    // Now that the child's fees have "increased" by 1 BBTC, the cheap package should succeed.
    {
    ...
    // Parent pays 1 BBTC and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
