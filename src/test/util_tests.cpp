...
CKey KeyFromBytes(const std::array<unsigned char, 32>& privkey_bytes, bool compressed)
{
    CKey privkey;
    privkey.Set(privkey_bytes.begin(), privkey_bytes.end(), compressed);
    return privkey;
}
} // namespace
...
{
    const std::array<unsigned char, 32> compressed_key_bytes = {
        0xD9, 0x7F, 0x51, 0x08, 0xF1, 0x1C, 0xDA, 0x6E,
        0xEE, 0xBA, 0xAA, 0x42, 0x0F, 0xEF, 0x07, 0x26,
        0xB1, 0xF8, 0x98, 0x06, 0x0B, 0x98, 0x48, 0x9F,
        0xA3, 0x09, 0x84, 0x63, 0xC0, 0x03, 0x28, 0x66
    };
    const std::array<unsigned char, 32> uncompressed_key_bytes = {
    ...
    const CKey compressed_key = KeyFromBytes(compressed_key_bytes, true);
    const CKey uncompressed_key = KeyFromBytes(uncompressed_key_bytes, false);
    BOOST_REQUIRE(compressed_key.IsValid());
    BOOST_REQUIRE(uncompressed_key.IsValid());

    const std::string compressed_address = EncodeDestination(PKHash(compressed_key.GetPubKey()));
    const std::string no_key_address = EncodeDestination(ScriptHash(CScript{} << OP_TRUE));
    std::string compressed_signature;
    BOOST_REQUIRE(MessageSign(compressed_key, "Trust no one", compressed_signature));
