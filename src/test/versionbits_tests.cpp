...
static void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    int32_t expected_base_version = VERSIONBITS_TOP_BITS;
    if (params.nAuxpowChainId != 0) {
        expected_base_version = CPureBlockHeader::GetBaseVersion(expected_base_version) |
                                ((params.nAuxpowChainId & 0xFF) * CPureBlockHeader::VERSION_CHAIN_START);
    }

    // should not be any deployment signalling for first block
    BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(nullptr, params), expected_base_version);
