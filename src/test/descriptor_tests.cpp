...
bool HasSingleDescriptorChecksum(const std::string& desc)
{
    return desc.size() > 9 && desc[desc.size() - 9] == '#' && desc.find('#') == desc.size() - 9;
}
...
std::string ConvertDescriptorBodyBase58Checks(const std::string& desc)
{
    std::string converted;
    converted.reserve(desc.size());

    for (size_t i = 0; i < desc.size();) {
        if (BASE58_CHARS.find(desc[i]) == std::string::npos) {
            converted.push_back(desc[i]);
            ++i;
            continue;
        }

        size_t j = i;
        while (j < desc.size() && BASE58_CHARS.find(desc[j]) != std::string::npos) {
            ++j;
        }

        std::string token = desc.substr(i, j - i);
        if (token.size() >= 20) {
...
std::string ConvertDescriptorFixture(const std::string& desc, bool rewrite_checksum)
{
    if (!HasSingleDescriptorChecksum(desc)) {
        return ConvertDescriptorBodyBase58Checks(desc);
    }

    const std::string converted_body = ConvertDescriptorBodyBase58Checks(desc.substr(0, desc.size() - 9));
    if (!rewrite_checksum) {
        return converted_body + desc.substr(desc.size() - 9);
    }
    const std::string checksum = GetDescriptorChecksum(converted_body);
    if (checksum.empty()) {
        return converted_body + desc.substr(desc.size() - 9);
    }
    return converted_body + "#" + checksum;
}

std::string RewriteChecksumMismatchError(const std::string& expected_error, const std::string& converted_desc)
{
    constexpr char prefix[] = "Provided checksum '";
    if (expected_error.rfind(prefix, 0) != 0 || !HasSingleDescriptorChecksum(converted_desc)) {
        return expected_error;
    }

    const size_t first_quote = expected_error.find('\'');
    const size_t second_quote = expected_error.find('\'', first_quote + 1);
    if (first_quote == std::string::npos || second_quote == std::string::npos) {
        return expected_error;
    }

    const std::string provided = expected_error.substr(first_quote + 1, second_quote - first_quote - 1);
    const std::string body = converted_desc.substr(0, converted_desc.size() - 9);
    return "Provided checksum '" + provided + "' does not match computed checksum '" + GetDescriptorChecksum(body) + "'";
}

void CheckUnparsable(const std::string& prv, const std::string& pub, const std::string& expected_error)
{
    const bool checksum_mismatch = expected_error.rfind("Provided checksum '", 0) == 0;
    const std::string converted_prv = ConvertDescriptorFixture(prv, /*rewrite_checksum=*/!checksum_mismatch);
    const std::string converted_pub = ConvertDescriptorFixture(pub, /*rewrite_checksum=*/!checksum_mismatch);

    FlatSigningProvider keys_priv, keys_pub;
    std::string error;
    auto parse_priv = Parse(converted_prv, keys_priv, error);
    auto parse_pub = Parse(converted_pub, keys_pub, error);
    BOOST_CHECK_MESSAGE(!parse_priv, converted_prv);
    BOOST_CHECK_MESSAGE(!parse_pub, converted_pub);
    BOOST_CHECK_EQUAL(error, RewriteChecksumMismatchError(ConvertDescriptorBodyBase58Checks(expected_error), converted_pub));
}
...
{
    const std::string converted_prv = ConvertDescriptorFixture(prv, /*rewrite_checksum=*/true);
    const std::string converted_pub = ConvertDescriptorFixture(pub, /*rewrite_checksum=*/true);
    const std::string converted_norm_pub = ConvertDescriptorFixture(norm_pub, /*rewrite_checksum=*/true);
    ...
    if (replace_apostrophe_with_h_in_prv) {
        parse_priv = Parse(UseHInsteadOfApostrophe(converted_prv), keys_priv, error);
    } else {
        parse_priv = Parse(converted_prv, keys_priv, error);
    }
    BOOST_CHECK_MESSAGE(parse_priv, error);
    if (replace_apostrophe_with_h_in_pub) {
        parse_pub = Parse(UseHInsteadOfApostrophe(converted_pub), keys_pub, error);
    } else {
        parse_pub = Parse(converted_pub, keys_pub, error);
    }
    ...
    BOOST_CHECK_MESSAGE(EqualDescriptor(converted_pub, pub1), "Private ser: " + pub1 + " Public desc: " + converted_pub);
    BOOST_CHECK_MESSAGE(EqualDescriptor(converted_pub, pub2), "Public ser: " + pub2 + " Public desc: " + converted_pub);

    // Check that both can be serialized with private key back to the private version, but not without private key.
    if (!(flags & MISSING_PRIVKEYS)) {
        std::string prv1;
        BOOST_CHECK(parse_priv->ToPrivateString(keys_priv, prv1));
        BOOST_CHECK(EqualDescriptor(converted_prv, prv1));
    ...
    BOOST_CHECK(EqualDescriptor(norm1, converted_norm_pub));
