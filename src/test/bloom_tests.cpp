...
static uint256 GetTxid(const CBlock& block, size_t index)
{
    return block.vtx.at(index)->GetHash();
}

static void NormalizeMerkleRoot(CBlock& block)
{
    block.hashMerkleRoot = BlockMerkleRoot(block);
}

static void NormalizeBlock2Fixture(CBlock& block)
{
    CMutableTransaction tx2{*block.vtx.at(2)};
    for (CTxIn& txin : tx2.vin) {
        txin.prevout.hash = block.vtx.at(1)->GetHash();
    }
    block.vtx[2] = MakeTransactionRef(tx2);

    CMutableTransaction tx3{*block.vtx.at(3)};
    for (CTxIn& txin : tx3.vin) {
        txin.prevout.hash = block.vtx.at(2)->GetHash();
    }
    block.vtx[3] = MakeTransactionRef(tx3);

    NormalizeMerkleRoot(block);
}
...
BOOST_AUTO_TEST_CASE(bloom_match)
{
    ...
    const std::vector<unsigned char> tx_hash_bytes(tx_hash.begin(), tx_hash.end());
    ...
    CMutableTransaction spending_tx_mut;
    spendStream >> spending_tx_mut;
    spending_tx_mut.vin[0].prevout.hash = tx_hash;
    const CTransaction spendingTx{spending_tx_mut};
...
BOOST_AUTO_TEST_CASE(merkle_block_1)
{
    ...
    const uint256 last_txid = GetTxid(block, 8);
    const uint256 eighth_txid = GetTxid(block, 7);
    CBloomFilter filter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    // Match the last transaction
    filter.insert(last_txid);
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[0].second == last_txid);
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[0].second == eighth_txid);
...
BOOST_AUTO_TEST_CASE(merkle_block_2)
{
    ...
    stream >> block;
    NormalizeBlock2Fixture(block);
    const uint256 tx0 = GetTxid(block, 0);
    const uint256 tx1 = GetTxid(block, 1);
    const uint256 tx2 = GetTxid(block, 2);
    const uint256 tx3 = GetTxid(block, 3);
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[0].second == tx0);
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[1].second == tx1);
    BOOST_CHECK(merkleBlock.vMatchedTxn[1].first == 1);

    BOOST_CHECK(merkleBlock.vMatchedTxn[2].second == tx2);
    BOOST_CHECK(merkleBlock.vMatchedTxn[2].first == 2);

    BOOST_CHECK(merkleBlock.vMatchedTxn[3].second == tx3);
...
BOOST_AUTO_TEST_CASE(merkle_block_2_with_update_none)
{
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[2].second == tx3);
...
BOOST_AUTO_TEST_CASE(merkle_block_3_and_serialize)
{
    ...
    stream >> block;
    NormalizeMerkleRoot(block);
    const uint256 only_txid = GetTxid(block, 0);

    CBloomFilter filter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    // Match the only transaction
    filter.insert(only_txid);
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[0].second == only_txid);
    BOOST_CHECK(merkleBlock.vMatchedTxn[0].first == 0);
    ...
    merkleStream >> roundtrip;
    BOOST_CHECK(roundtrip.header.GetHash() == merkleBlock.header.GetHash());
    std::vector<uint256> roundtrip_vMatched;
    std::vector<unsigned int> roundtrip_vIndex;
    BOOST_CHECK(roundtrip.txn.ExtractMatches(roundtrip_vMatched, roundtrip_vIndex) == merkleBlock.header.hashMerkleRoot);
    BOOST_CHECK_EQUAL(roundtrip_vMatched.size(), 1U);
    BOOST_CHECK_EQUAL(roundtrip_vIndex.size(), 1U);
    BOOST_CHECK(roundtrip_vMatched[0] == only_txid);
    BOOST_CHECK_EQUAL(roundtrip_vIndex[0], 0U);
}
...
BOOST_AUTO_TEST_CASE(merkle_block_4)
{
    ...
    stream >> block;
    NormalizeMerkleRoot(block);
    const uint256 last_txid = GetTxid(block, 6);
    const uint256 fourth_txid = GetTxid(block, 3);
    ...
    filter.insert(fourth_txid);
    merkleBlock = CMerkleBlock(block, filter);
    BOOST_CHECK(merkleBlock.header.GetHash() == block.GetHash());

    BOOST_CHECK(merkleBlock.vMatchedTxn.size() == 2);

    BOOST_CHECK(merkleBlock.vMatchedTxn[0].second == fourth_txid);
...
BOOST_AUTO_TEST_CASE(merkle_block_4_test_p2pubkey_only)
{
    ...
    stream >> block;
    NormalizeMerkleRoot(block);
    const uint256 coinbase_txid = GetTxid(block, 0);
    const uint256 fourth_txid = GetTxid(block, 3);
    ...
    BOOST_CHECK(filter.contains(COutPoint(coinbase_txid, 0)));
    // ... but not the 4th transaction's output (its not pay-2-pubkey)
    BOOST_CHECK(!filter.contains(COutPoint(fourth_txid, 0)));
}
