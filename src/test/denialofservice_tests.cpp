...
BOOST_AUTO_TEST_CASE(stale_tip_peer_management)
{
    ...
    const auto stale_tip_delay{3 * std::chrono::seconds{m_node.chainman->GetConsensus().nPowTargetSpacing} + 1s};
    const auto stale_check_delay{std::chrono::minutes{10} + 1s};
    const auto time_later{time_init + (stale_tip_delay > stale_check_delay ? stale_tip_delay : stale_check_delay)};
