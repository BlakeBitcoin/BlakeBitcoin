...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    // BlakeBitcoin 0.25.2 enforces relative locks during block assembly here, so
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    static_assert(std::size(BLOCK_EXTRANONCES) == 110, "Should have 110 blocks to import");
    int baseheight = 0;
    std::vector<CTransactionRef> txFirst;
    for (const auto extranonce : BLOCK_EXTRANONCES) {
        pblocktemplate = AssemblerForTest(tx_mempool).CreateNewBlock(scriptPubKey);
        ...
        CBlock *pblock = &pblocktemplate->block; // pointer for convenience
        {
            ...
            // BlakeBitcoin regtest starts from the historical genesis nBits. Push
