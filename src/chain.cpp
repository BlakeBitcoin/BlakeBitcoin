...
CBlockHeader CBlockIndex::GetBlockHeader(const node::BlockManager& blockman) const
{
    ...
    // AuxPoW headers need their payload for p2p/RPC header serialization.
    // Prefer blk*.dat when available so freshly read full-block data is used,
    // but fall back to the block-index copy so pruned nodes can still serve
    // valid historical AuxPoW headers.
    if (block.IsAuxpow()) {
        CBlockHeader disk_header;
        if (blockman.ReadBlockHeader(disk_header, *this)) {
            return disk_header;
        }
    }
