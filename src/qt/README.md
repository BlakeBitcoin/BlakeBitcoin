This directory contains the source code for the BlakeBitcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the BlakeBitcoin UI.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a blakebitcoin:... link from a browser).
...
* UI amount fields, which inherit from QWidget.
* Generated UI string sources
* BBTC / mBBTC / etc. unit handling
* `callback.h`
...
**Note:** Do not change the generated English translation file under `locale/`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `blakebitcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `blakebitcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `blakebitcoin-qt`
9. You're all set! Start developing, building, and debugging the BlakeBitcoin Core GUI
