...
class BlockchainTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.time_range_mtp = None
        self.time_range_tip = None
    ...
    def mine_chain(self):
        ...
        for t in range(genesis_time + TIME_RANGE_STEP, genesis_time + ((HEIGHT + 1) * TIME_RANGE_STEP), TIME_RANGE_STEP):
            self.nodes[0].setmocktime(t)
            self.generate(self.wallet, 1)
        assert_equal(self.nodes[0].getblockchaininfo()['blocks'], HEIGHT)
        info = self.nodes[0].getblockchaininfo()
        self.time_range_tip = info['time']
        self.time_range_mtp = info['mediantime']
    ...
    def _test_max_future_block_time(self):
        ...
        self.nodes[0].assert_start_raises_init_error(
            extra_args=[f"-mocktime={self.time_range_tip - MAX_FUTURE_BLOCK_TIME - 1}"],
        ...
        self.start_node(0, extra_args=[f"-mocktime={self.time_range_tip - MAX_FUTURE_BLOCK_TIME}"])
    ...
    def _test_getblockchaininfo(self):
        ...
        assert_equal(res['time'], self.time_range_tip)
        assert_equal(res['mediantime'], self.time_range_mtp)
    ...
    def _test_getblockheader(self):
        ...
        assert_equal(header['mediantime'], self.time_range_mtp)
    ...
    def _test_getblock(self):
        ...
        def assert_hexblock_hashes(verbosity):
            ...
            assert_equal(blockhash, blake256(bytes.fromhex(block[:160]))[::-1].hex())
