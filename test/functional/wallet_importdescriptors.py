...
class ImportDescriptorsTest(BitcoinTestFramework):
    ...
    def derive_desc_addresses(self, desc, addr_range=None):
        descriptor = descsum_create(desc)
        if addr_range is None:
            return self.nodes[1].deriveaddresses(descriptor)
        return self.nodes[1].deriveaddresses(descriptor, addr_range)
    ...
    def run_test(self):
        ...
        xpriv = translate_testnet_extkey("tprv8ZgxMBicQKsPeuVhWwi6wuMQGfPKi9Li5GtX35jVNknACgqe3CY4g5xgkfDDJcmtF7o1QnxWDRYw4H5P26PXq7sbcUkEqeR4fg3Kxp2tigg")
        xpub = translate_testnet_extkey("tpubD6NzVbkrYhZ4YNXVQbNhMK1WqguFsUXceaVJKbmno2aZ3B6QfbMeraaYvnBSGpV3vxLyTTK9DYT1yoEck4XUScMzXoQ2U2oSmE2JyMedq3H")
        wrapped_addresses = self.derive_desc_addresses("sh(wpkh(" + xpub + "/0/0/*))", [0, 1])
        plain_addresses = self.derive_desc_addresses("wpkh(" + xpub + "/0/0/*)", [0, 1])
        ...
        for address in wrapped_addresses:
            test_address(w1,
                         address,
                         ismine=True,
                         solvable=True)
        ...
        xpub = translate_testnet_extkey("tpubDAXcJ7s7ZwicqjprRaEWdPoHKrCS215qxGYxpusRLLmJuT69ZSicuGdSfyvyKpvUNYBW1s2U3NSrT6vrCYB9e6nZUEvrqnwXPF8ArTCRXMY")
        addresses = self.derive_desc_addresses("wpkh([80002067/0h/0h]" + xpub + "/*)", [0, 4])
        ...
        assert_equal(address, self.derive_desc_addresses("pkh([12345678]" + xpub + "/*)", [0, 0])[0])
        ...
        wif_priv = translate_testnet_wif("cTe1f5rdT8A8DFgVWTjyPwACsDPJM9ff4QngFxUixCSvvbg1x6sh")
        desc = "sh(wpkh(" + wif_priv + "))"
        address = self.derive_desc_addresses(desc)[0]
        ...
        xprv1 = translate_testnet_extkey('tprv8ZgxMBicQKsPevADjDCWsa6DfhkVXicu8NQUzfibwX2MexVwW4tCec5mXdCW8kJwkzBRRmAay1KZya4WsehVvjTGVW6JLqiqd8DdZ4xSg52')
        acc_xpub1 = translate_testnet_extkey('tpubDCJtdt5dgJpdhW4MtaVYDhG4T4tF6jcLR1PxL43q9pq1mxvXgMS9Mzw1HnXG15vxUGQJMMSqCQHMTy3F1eW5VkgVroWzchsPD5BUojrcWs8')  # /84'/0'/0'
        chg_xpub1 = translate_testnet_extkey('tpubDCXqdwWZcszwqYJSnZp8eARkxGJfHAk23KDxbztV4BbschfaTfYLTcSkSJ3TN64dRqwa1rnFUScsYormKkGqNbbPwkorQimVevXjxzUV9Gf')  # /84'/1'/0'
        xprv2 = translate_testnet_extkey('tprv8ZgxMBicQKsPdSNWUhDiwTScDr6JfkZuLshTRwzvZGnMSnGikV6jxpmdDkC3YRc4T3GD6Nvg9uv6hQg73RVv1EiTXDZwxVbsLugVHU8B1aq')
        acc_xprv2 = translate_testnet_extkey('tprv8gVCsmRAxVSxyUpsL13Y7ZEWBFPWbgS5E2MmFVNGuANrknvmmn2vWnmHvU8AwEFYzR2ji6EeZLSCLVacsYkvor3Pcb5JY5FGcevqTwYvdYx')
        acc_xpub2 = translate_testnet_extkey('tpubDDBF2BTR6s8drwrfDei8WxtckGuSm1cyoKxYY1QaKSBFbHBYQArWhHPA6eJrzZej6nfHGLSURYSLHr7GuYch8aY5n61tGqgn8b4cXrMuoPH')
        chg_xpub2 = translate_testnet_extkey('tpubDCYfZY2ceyHzYzMMVPt9MNeiqtQ2T7Uyp9QSFwYXh8Vi9iJFYXcuphJaGXfF3jUQJi5Y3GMNXvM11gaL4txzZgNGK22BFAwMXynnzv4z2Jh')
        xprv3 = translate_testnet_extkey('tprv8ZgxMBicQKsPeonDt8Ka2mrQmHa61hQ5FQCsvWBTpSNzBFgM58cV2EuXNAHF14VawVpznnme3SuTbA62sGriwWyKifJmXntfNeK7zeqMCj1')
        acc_xpub3 = translate_testnet_extkey('tpubDCsWoW1kuQB9kG5MXewHqkbjPtqPueRnXju7uM2NK7y3JYb2ajAZ9EiuZXNNuE4661RAfriBWhL8UsnAPpk8zrKKnZw1Ug7X4oHgMdZiU4E')
        chg_xpub3 = translate_testnet_extkey('tpubDC6UGqnsQStngYuGD4MKsMy7eD1Yg9NTJfPdvjdG2JE5oZ7EsSL3WHg4Gsw2pR5K39ZwJ46M1wZayhedVdQtMGaUhq5S23PH6fnENK3V1sb')
        wmulti_priv_recv_desc = "wsh(multi(2," + xprv1 + "/84h/0h/0h/*," + xprv2 + "/84h/0h/0h/*," + xprv3 + "/84h/0h/0h/*))"
        wmulti_priv_change_desc = "wsh(multi(2," + xprv1 + "/84h/1h/0h/*," + xprv2 + "/84h/1h/0h/*," + xprv3 + "/84h/1h/0h/*))"
        wmulti_pub_recv_desc = "wsh(multi(2,[7b2d0242/84h/0h/0h]" + acc_xpub1 + "/*,[59b09cd6/84h/0h/0h]" + acc_xpub2 + "/*,[e81a0532/84h/0h/0h]" + acc_xpub3 + "/*))"
        wmulti_pub_change_desc = "wsh(multi(2,[7b2d0242/84h/1h/0h]" + chg_xpub1 + "/*,[59b09cd6/84h/1h/0h]" + chg_xpub2 + "/*,[e81a0532/84h/1h/0h]" + chg_xpub3 + "/*))"

        self.test_importdesc({"desc": descsum_create(wmulti_priv_recv_desc),
                            "active": True,
                            "range": 1000,
                            "next_index": 0,
                            "timestamp": "now"},
                            success=True,
                            wallet=wmulti_priv)
        self.test_importdesc({"desc": descsum_create(wmulti_priv_change_desc),
                            "active": True,
                            "internal" : True,
                            "range": 1000,
                            "next_index": 0,
        ...
        assert_equal(addr, self.derive_desc_addresses(wmulti_priv_recv_desc, [0, 0])[0]) # Derived at m/84'/0'/0'/0
        change_addr = wmulti_priv.getrawchangeaddress('bech32') # uses change 0
        assert_equal(change_addr, self.derive_desc_addresses(wmulti_priv_change_desc, [0, 0])[0]) # Derived at m/84'/1'/0'/0
        ...
        self.test_importdesc({"desc": descsum_create(wmulti_pub_recv_desc),
                            "active": True,
                            "range": 1000,
                            "next_index": 0,
                            "timestamp": "now"},
                            success=True,
                            wallet=wmulti_pub)
        self.test_importdesc({"desc": descsum_create(wmulti_pub_change_desc),
                            "active": True,
                            "internal" : True,
                            "range": 1000,
                            "next_index": 0,
        ...
        assert_equal(addr, self.derive_desc_addresses(wmulti_pub_recv_desc, [1, 1])[0]) # Derived at m/84'/0'/0'/1
        change_addr = wmulti_pub.getrawchangeaddress('bech32') # uses change 2
        assert_equal(change_addr, self.derive_desc_addresses(wmulti_pub_change_desc, [2, 2])[0]) # Derived at m/84'/1'/0'/2
        ...
        xkey_root = translate_testnet_extkey("tprv8ZgxMBicQKsPeZSeYx7VXDDTs3XrTcmZQpRLbAeSQFCQGgKwR4gKpcxHaKdoTNHniv4EPDJNdzA3KxRrrBHcAgth8fU5X4oCndkkxk39iAt")
        xkey = xkey_root + "/*"
        xkey_int = xkey_root + "/1/*"
        ...
        self.test_importdesc({"desc": descsum_create("combo(" + acc_xpub1 + "/*)"),
                              "active": True,
                              "range": 1,
                              "timestamp": "now"},
                              success=False,
        ...
        self.test_importdesc({"desc": descsum_create("pk(" + acc_xpub1 + "/*)"),
                              "active": True,
                              "range": 1,
                              "timestamp": "now"},
                              success=True,
        ...
        rescan_start = f"Rescan started from block {self.nodes[0].getblockhash(0)}... (slow variant inspecting all blocks)"
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as thread:
            with self.nodes[0].assert_debug_log(expected_msgs=[rescan_start], timeout=5):
