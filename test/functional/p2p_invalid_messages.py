...
class InvalidMessagesTest(BitcoinTestFramework):
    ...
    def test_checksum(self):
        ...
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        badmsg = msg_unrecognized(str_data="d")
        expected_checksum = blake256(badmsg.serialize())[:4].hex()
        with self.nodes[0].assert_debug_log([f'Header error: Wrong checksum (badmsg, 2 bytes), expected {expected_checksum} was ffffffff']):
            msg = conn.build_message(badmsg)
    ...
    def test_invalid_pow_headers_msg(self):
        ...
        block.solve()
        blockheader = CBlockHeader(block)
        ...
        # invalidate PoW
        while blockheader.sha256 <= target:
