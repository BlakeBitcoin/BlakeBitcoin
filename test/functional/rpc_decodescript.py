...
class DecodeScriptTest(BitcoinTestFramework):
    ...
    def translate_testnet_address(self, address):
    ...
    def translate_expected_desc(self, desc):
        if desc.startswith("addr(") and ")" in desc:
            address = desc[5:desc.index(")")]
            return descsum_create(f"addr({self.translate_testnet_address(address)})")
        return desc

    def translate_expected_result(self, value):
        if isinstance(value, list):
            return [self.translate_expected_result(item) for item in value]
        if isinstance(value, dict):
            translated = {}
            for key, item in value.items():
                if key in {"address", "p2sh", "p2sh-segwit"}:
                    translated[key] = self.translate_testnet_address(item)
                elif key == "desc":
                    translated[key] = self.translate_expected_desc(item)
                else:
                    translated[key] = self.translate_expected_result(item)
            return translated
        return value
    ...
    def decoderawtransaction_asm_sighashtype(self):
        ...
        tx_obj.calc_sha256()
        assert_equal(tx_obj.hash, rpc_result['txid'])
    ...
    def decodescript_datadriven_tests(self):
        ...
        for script, result in dd_tests:
            ...
            assert_equal(self.translate_expected_result(result), rpc_result)
    ...
    def decodescript_miniscript(self):
        ...
        assert res["segwit"]["desc"] == descsum_create(f"addr({self.translate_testnet_address('bcrt1q73qyfypp47hvgnkjqnav0j3k2lq3v76wg22dk8tmwuz5sfgv66xsvxg6uu')})")
