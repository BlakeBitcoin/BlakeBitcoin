...
def enable_all_mempool_probes(ctx):
    """Enable every probe referenced by MEMPOOL_TRACEPOINTS_PROGRAM.

    bcc only generates bpf_usdt_readarg helper stubs for probes enabled on the
    USDT context before compilation. This test compiles one shared C program
    containing handlers for all mempool tracepoints, so enable them together to
    keep helper generation deterministic across toolchains.
    """
