...
def assert_fee_amount(fee, tx_size, feerate_BBTC_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_BBTC_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s BBTC too low! (Should be %s BBTC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BBTC_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s BBTC too high! (Should be %s BBTC)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting BBTC values"""
...
def get_fee(tx_size, feerate_bbtc_kvb):
    """Calculate the fee in BBTC given a feerate is BBTC/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_bbtc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in BBTC
...
CONFIG_FILENAME = "blakebitcoin.conf"
