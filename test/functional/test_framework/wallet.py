...
class MiniWallet:
    ...
    def scan_tx(self, tx):
        ...
        for spent in tx["vin"]:
            ...
            try:
                self.get_utxo(txid=spent["txid"], vout=spent["vout"], include_immature_coinbase=True)
    ...
    def get_utxo(self, *, txid: str = '', vout: Optional[int] = None, mark_as_spent=True, include_immature_coinbase=False, current_height=None) -> dict:
        """
        ...
        if include_immature_coinbase:
            utxo_filter: Any = self._utxos
        else:
            spend_height = (self._test_node.getblockcount() if current_height is None else current_height) + 1
            utxo_filter = filter(lambda utxo: not utxo['coinbase'] or coinbase_maturity(spend_height) <= spend_height - utxo['height'], self._utxos)
        if txid:
            utxo_filter = filter(lambda utxo: txid == utxo['txid'], utxo_filter)
        else:
            utxo_filter = reversed(list(utxo_filter))  # By default the largest utxo
        if vout is not None:
    ...
    def get_utxos(self, *, include_immature_coinbase=False, mark_as_spent=True, current_height=None):
    ...
    def send_self_transfer(self, *, from_node, **kwargs):
        ...
        kwargs.setdefault("current_height", from_node.getblockcount())
    ...
    def send_to(self, *, from_node, scriptPubKey, amount, fee=1000):
        ...
        """
        tx = self.create_self_transfer(fee_rate=0, current_height=from_node.getblockcount())["tx"]
    ...
    def create_self_transfer_multi(
        ...
        *,
        ...
        """
        utxos_to_spend = utxos_to_spend or [self.get_utxo(current_height=current_height)]
    ...
    def create_self_transfer(self, *, fee_rate=Decimal("0.003"), fee=Decimal("0"), utxo_to_spend=None, locktime=0, sequence=0, target_weight=0, current_height=None):
        """Create and return a tx with the specified fee. If fee is 0, use fee_rate, where the resulting fee may be exact or at most one satoshi higher than needed."""
        utxo_to_spend = utxo_to_spend or self.get_utxo(current_height=current_height)
        assert fee_rate >= 0
        assert fee >= 0
        # calculate fee
