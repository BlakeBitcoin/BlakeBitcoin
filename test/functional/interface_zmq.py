...
def sha256_reversed(byte_str):
    return sha256(byte_str)[::-1]


def blake256_reversed(byte_str):
    return blake256_hash(byte_str)[::-1]
...
class ZMQTest (BitcoinTestFramework):
    ...
    def test_basic(self):
        ...
        for x in range(num_blocks):
            ...
            assert_equal(genhashes[x], blake256_reversed(block[:80]).hex())
        ...
        hex = rawtx.receive()
        assert_equal(payment_tx['wtxid'], sha256_reversed(hex).hex())
