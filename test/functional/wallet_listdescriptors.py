...
class ListDescriptorsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        xprv = translate_testnet_extkey('tprv8ZgxMBicQKsPeuVhWwi6wuMQGfPKi9Li5GtX35jVNknACgqe3CY4g5xgkfDDJcmtF7o1QnxWDRYw4H5P26PXq7sbcUkEqeR4fg3Kxp2tigg')
        xpub_acc = translate_testnet_extkey('tpubDCMVLhErorrAGfApiJSJzEKwqeaf2z3NrkVMxgYQjZLzMjXMBeRw2muGNYbvaekAE8rUFLftyEar4LdrG2wXyyTJQZ26zptmeTEjPTaATts')
        ...
        combo_privkey = node.get_deterministic_priv_key().key
        wallet.importdescriptors([{
            'desc': descsum_create('combo(' + combo_privkey + ')'),
            'timestamp': TIME_GENESIS_BLOCK,
        }])
        expected = {
            'wallet_name': 'w4',
            'descriptors': [
                {'active': False,
                 'desc': node.getdescriptorinfo('combo(' + combo_privkey + ')')['descriptor'],
                 'timestamp': TIME_GENESIS_BLOCK},
            ]
        }
        assert_equal(expected, wallet.listdescriptors())
