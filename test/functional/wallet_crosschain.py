...
class WalletCrossChain(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        distinct_genesis = self.nodes[0].getblockhash(0) != self.nodes[1].getblockhash(0)
        descriptor_cross_chain = self.options.descriptors and self.nodes[0].chain != self.nodes[1].chain
        cross_chain = distinct_genesis or descriptor_cross_chain
        ...
        if not self.options.descriptors and distinct_genesis:
