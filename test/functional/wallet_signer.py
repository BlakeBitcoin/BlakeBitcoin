...
from test_framework.address import translate_testnet_extkey
...
class WalletSignerTest(BitcoinTestFramework):
    ...
    def test_valid_signer(self):
        ...
        signer_xpub = translate_testnet_extkey("tpubD6NzVbkrYhZ4WaWSyoBvQwbpLkojyoTZPRsgXELWz3Popb3qkjcJyJUGLnL4qHHoQvao8ESaAstxYSnhyswJ76uZPStJRJCTKvosUCJZL5B")
        ...
        expected_bech32 = self.nodes[1].deriveaddresses(descsum_create(f"wpkh([00000001/84'/1'/0']{signer_xpub}/0/*)"), [0, 0])[0]
        expected_p2sh = self.nodes[1].deriveaddresses(descsum_create(f"sh(wpkh([00000001/49'/1'/0']{signer_xpub}/0/*))"), [0, 0])[0]
        expected_legacy = self.nodes[1].deriveaddresses(descsum_create(f"pkh([00000001/44'/1'/0']{signer_xpub}/0/*)"), [0, 0])[0]
        expected_bech32m = self.nodes[1].deriveaddresses(descsum_create(f"tr([00000001/86'/1'/0']{signer_xpub}/0/*)"), [0, 0])[0]

        address1 = hww.getnewaddress(address_type="bech32")
        assert_equal(address1, expected_bech32)
        ...
        assert_equal(address2, expected_p2sh)
        ...
        assert_equal(address3, expected_legacy)
        ...
        assert_equal(address4, expected_bech32m)
        ...
        signer_xprv = translate_testnet_extkey("tprv8ZgxMBicQKsPd7Uf69XL1XwhmjHopUGep8GuEiJDZmbQz6o58LninorQAfcKZWARbtRtfnLcJ5MQ2AtHcQJCCRUcMRvmDUjyEmNUWwx8UbK")

        result = mock_wallet.importdescriptors([{
            "desc": descsum_create(f"tr([00000001/86'/1'/0']{signer_xprv}/0/*)"),
            "timestamp": 0,
            "range": [0,1],
            "internal": False,
            "active": True
        ...
        {
            "desc": descsum_create(f"tr([00000001/86'/1'/0']{signer_xprv}/1/*)"),
            "timestamp": 0,
            "range": [0, 0],
            "internal": True,
            "active": True
